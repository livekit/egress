...
func BuildAudioBin(pipeline *gstreamer.Pipeline, p *config.PipelineConfig) error {
	...
	if len(p.GetEncodedOutputs()) > 1 {
		...
		queue, err := gstreamer.BuildQueue("audio_queue", uint64(p.Latency.PipelineLatency), true)
		if err != nil {
...
func addAudioConverter(b *gstreamer.Bin, p *config.PipelineConfig, channel int) error {
	audioQueue, err := gstreamer.BuildQueue("audio_input_queue", uint64(p.Latency.PipelineLatency), true)
	if err != nil {
		return err
	}
