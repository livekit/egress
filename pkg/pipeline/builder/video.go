...
func BuildVideoBin(pipeline *gstreamer.Pipeline, p *config.PipelineConfig) error {
	...
	if len(p.GetEncodedOutputs()) > 1 {
		...
		queue, err := gstreamer.BuildQueue("video_queue", uint64(p.Latency.PipelineLatency), true)
		if err != nil {
			return errors.ErrGstPipelineError(err)
		}
...
func (b *VideoBin) buildWebInput() error {
	...
	videoQueue, err := gstreamer.BuildQueue("video_input_queue", uint64(b.conf.Latency.PipelineLatency), true)
	if err != nil {
		return errors.ErrGstPipelineError(err)
	}
...
func (b *VideoBin) addVideoTestSrcBin() error {
	...
	queue, err := gstreamer.BuildQueue("video_test_src_queue", uint64(b.conf.Latency.PipelineLatency), false)
	if err != nil {
		return err
	}
...
func (b *VideoBin) addEncoder() error {
	videoQueue, err := gstreamer.BuildQueue("video_encoder_queue", uint64(b.conf.Latency.PipelineLatency), false)
	if err != nil {
		return errors.ErrGstPipelineError(err)
	}
