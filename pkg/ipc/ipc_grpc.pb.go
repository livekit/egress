...
func (c *egressHandlerClient) StopHandler(ctx context.Context, in *StopHandlerRequest, opts ...grpc.CallOption) (*emptypb.Empty, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(emptypb.Empty)
	err := c.cc.Invoke(ctx, EgressHandler_StopHandler_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func (UnimplementedEgressHandlerServer) StopHandler(context.Context, *StopHandlerRequest) (*emptypb.Empty, error) {
...
func RegisterEgressHandlerServer(s grpc.ServiceRegistrar, srv EgressHandlerServer) {
	// If the following call panics, it indicates UnimplementedEgressHandlerServer was
	// embedded by pointer and is nil.  This will cause panics if an
	// unimplemented method is ever invoked, so we test this at initialization
	// time to prevent it from happening at runtime later due to I/O.
	if t, ok := srv.(interface{ testEmbeddedByValue() }); ok {
...
func _EgressHandler_StopHandler_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(StopHandlerRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(EgressHandlerServer).StopHandler(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: EgressHandler_StopHandler_FullMethodName,
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(EgressHandlerServer).StopHandler(ctx, req.(*StopHandlerRequest))
	}
	return interceptor(ctx, in, info, handler)
}
...
var EgressHandler_ServiceDesc = grpc.ServiceDesc{
	...
	Methods: []grpc.MethodDesc{
		...
		},
		{
			...
			Handler:    _EgressHandler_StopHandler_Handler,
		},
		{
